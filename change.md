...
- **根因**：`2604.02176` 将“Translate the following sentence ...”示例模板直接
  排版；同时带 `\\ref` 的方括号标题被标题启发式误判为正文，触发无效翻译请求。
- **做减法**：扩展现有 inline source-data 谓词识别翻译提示模板，并在标题判断前
  去除引用命令；这些内容统一原样保留，不新增论文级特例或重试分支。
- **版本与验证**：chunk 边界升级为 `v56-prompt-boundary`，新增模板/引用标题回归
  测试；修复后继续重跑 `2604.02176`。

### 提示模板结构单元保护

- **根因**：模板中的 TeX 换行 `\\` 会被结构单元拆分器当成普通边界，导致前一轮
  的模板保护在切分后失效。
- **做减法**：结构单元拆分前直接识别完整提示模板并原样保留，避免新增重试逻辑。
- **版本**：chunk 边界升级为 `v58-prompt-lines`，跨物理行的模板最多保护 12 行。

## v4.43 — 2026-08-16

### 结构响应规范化与邮箱宏保护

- **根因**：自适应细分合并后，模型可能丢掉前置 `\\paragraph` 或尾部 `\\ref`；
  `\\emailtext{...}` 也会被误当成需要翻译的正文。
- **做减法**：统一在响应规范化阶段恢复源标题/引用；将邮箱宏归入结构元数据，
  不新增论文级重试分支。
- **版本与验证**：chunk 升级为 `v59-structure-normalize`，新增结构/邮箱回归测试；
  修复后继续重跑 `2604.02721`。
...
- **根因**：历史长文中的纯公式/编译命令被送进模型；citation 被 splitter 分离后，
  模型又会补写或漏掉 citation，导致 chunk 门禁失败。
- **做减法**：纯数学、编译命令、邮箱宏统一走结构数据保护；自适应合并后只按源
  citation 多重集做确定性补回/删除，不增加论文特例。
- **版本与验证**：chunk 升级为 `v60-structure-data`，新增回归测试；修复后继续
  重跑 `2602.10604`。

## v4.45 — 2026-08-16

### 论文内 grader/prompt 指令保护

- **根因**：`2602.10604` 直接排版 strict-grader 与 question-prompt 指令，模型将其
  当作翻译请求并回显/拒答，阻断整篇 PDF。
- **做减法**：将高置信 grader 指令和 prompt 前言纳入既有 source-data 谓词，原样
  保留，不新增论文级分支。
- **版本与验证**：chunk 升级为 `v61-prompt-directives`，新增回归测试；修复后继续
  重跑 `2602.10604`。

## v4.46 — 2026-08-16

### 最终输出指令、Java 命令与 URL 后引用边界修复

- **根因**：论文中的 `Your final output must be ...` 被当成待翻译正文；Java
  编译命令没有进入结构数据保护；脚注 URL 的行尾清理吞掉后续 citation，造成
  源/响应引用签名伪不一致。
- **做减法**：复用既有 source-data/编译命令谓词，并把 URL 清理限制在 URL
  本身，不增加论文级重试分支。
- **版本与验证**：chunk 升级为 `v62-source-boundaries`，新增回归测试；继续重跑
  `2602.10604`。
...
- **根因**：作者元数据中的纯 `\\href{email}{email}` 列表被拆成翻译 chunk，模型
  原样返回后触发漏译门禁。
- **做减法**：复用结构元数据谓词，仅保护完整邮箱链接目录，不扩大到普通 `\\href`
  或正文链接。
- **版本与验证**：chunk 升级为 `v63-email-links`，新增回归测试；继续重跑
  `2608.02602`。
...
- **根因**：作者列表末项使用自然语言 `and` 时，姓名目录谓词未拆开连接词，
  被误当成待翻译正文。
- **做减法**：在已有高置信姓名目录规则中统一归一化 `and`，不增加作者或论文
  特例分支。
- **版本与验证**：chunk 升级为 `v64-name-catalogs`，新增回归测试；继续重跑
  `2608.02711`。
...
- **根因**：通用“恢复丢失宏”逻辑把 `algorithmic.sty` 内部延迟定义的
  `\\STATE/\\IF/\\WHILE` 等命令复制进了文档导言区；`algorithmic` 环境随后再次
  `\\newcommand`，造成重复定义和 `\\ALC@it` 未定义。
- **做减法**：宏恢复只读取用户 `.tex` 源文件，不再扫描 `.sty/.cls` 实现文件；
  包/类的命令交给其自身生命周期定义，避免通用 patch 改变第三方包语义。
- **真实修复**：`2608.10905` 重新翻译覆盖率 85.1%，移除错误注入后复用 TeX
  编译成功（390KB），失败现场和 OPD 索引状态已清理并回写为 `ok`。
- **验证**：新增 package-environment 回归测试；相关 212 项测试通过。
...
- **2026-07-29 增量修复**：兼容 fallback 插入器现在同时跟踪跨行 `{}` 与
  ...
  均通过。质量门禁新增 TeX ``...'' 引用文本和命令密集 TikZ path 保护，
  避免把论文中的原始输入示例或 `path picture bounding box` 当成漏译正文；
  `2607.23420` 已在真实论文上验证并同步 paper store/topic 索引。
...
- **全量分析工具**：新增 `scripts/analyze_translation_chunks.py`，既能关联 gpt-academic `debug_log.html` 与翻译 TeX，统计 TRANSFORM 数量、长度、短 chunk、原文照抄和命令前缀，也能扫描全部成功/失败中文 TeX，输出英文词频、每行词数、CJK 覆盖区间、英文占主导/中英混合行、LaTeX 环境和命令分布。
- **历史分布**：扫描 393 份中文 TeX，发现 4,913 行长英文，其中 3,415 行英文占主导、1,525 行为中英混合；2,719 行含 8–15 个英文词、1,343 行含 16–30 个、851 行含 31 个以上。英文残留主要位于普通正文（3,651 行），其次为 itemize（263）、tcolorbox（217）、proof/abstract（各 96），共有 23 篇达到 `CJK < 50% 且长英文 > 20 行` 的历史严重阈值。
- **根因归纳**：保留的 13 份 splitter 调试样本共有 3,679 个 TRANSFORM chunk，其中 1,985 个短于 160 字符，1,324 个最终仍原样出现在中文 TeX。失败论文日志同时出现大量 chunk retry 和 429；上游默认 8 路请求把失败响应合并回英文原文并写入 `temp.pkl`，因而产出“能编译但正文未翻译”的半成品。
